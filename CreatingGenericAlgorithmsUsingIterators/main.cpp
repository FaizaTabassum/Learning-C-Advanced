...
//loops over two input iterators and sorts values 
template<class InputIterator, class OutputIterator>
OutputIterator mergeSortedContainer(InputIterator lhsBegin, InputIterator lhsEnd, InputIterator rhsBegin, InputIterator rhsEnd, OutputIterator result){
    //Copying elements retaining order until end of one mergeSortedContainer
...
//reversed copy - returns a reversed copy of the input container
...
OutputIterator reverseCopy(BidirectionalIterator iterBegin, BidirectionalIterator iterEnd, OutputIterator result){
    ...
    while(iterBegin != iterEnd){
        *result++ = *--iterEnd; // result zählt nach Zuordnung hoch, iterEnd wird direkt runtergezählt
    }
    return result;
}
...
void randomise(RandomAccessIterator iterBegin, RandomAccessIterator iterEnd){
    while (iterBegin < iterEnd){
        iter_swap(iterBegin, iterBegin+rand() % (iterEnd-iterBegin)); //randomInteger(iterEnd-iterBegin)); % means modulo
        ++iterBegin;
    }
}
...
int main(int argc, char **argv) {
	...
	mergeSortedContainer(firstList.begin(),firstList.end(),secondList.begin(),secondList.end(),resultsList.begin());
